...
app.post('/', passport.authenticate('login',{
		successRedirect: '/index_home',
		failureRedirect: '/',
		failureFlash: true
	}));
	
app.get('/settings', isLoggedIn, function(req, res){
		res.render('settings', {message: req.flash('signupMessage')});
	}); 
	
app.post('/settings', passport.authenticate('signup',{
		successRedirect: '/',
...
app.get('/index_home', isLoggedIn, function(req, res){
	 Todo.
    find().
    sort( '-updated_at' ).
...
app.get(  '/view_users', isLoggedIn, function(req, res){
		Todo.
    find().
    sort( '-updated_at' ).
    exec( function ( err, todos ){
...
app.get(  '/view_user_location', isLoggedIn, function(req, res){
	 Site.find()
	 .select('site_address state ihs_region latitude longitude project_id take_over_date ihs_site_id site_id')
	 .lean().exec(function (err, docs) {
		var dbOutput = './/public/ajax/db_output.json';
	var outValue = '{"data":' + JSON.stringify(docs, null, 4) + '}';
		fs.writeFile(dbOutput, outValue, function (err) {
			if (err) return console.log(err);
		console.log('Hello World > db_output.json');
});
		/* Site.
    find().
	exec( function ( err, sites ){
      res.render( 'view_user_location', {
          title : 'Express Todo Example',
          Sites : sites
      });
    }); */
...
app.get(  '/add_sites', isLoggedIn, function(req, res){
		Site.
    find().
    sort( '-updated_at' ).
    exec( function ( err, sites ){
...
app.get(  '/view_sites', isLoggedIn, function(req, res){
		Site.
    find().
    sort( '-updated_at' ).
    exec( function ( err, sites ){
...
app.get(  '/view_dgs', isLoggedIn, function(req, res){
		DGServices.
    find().
    sort( '-updated_at' ).
...
app.get(  '/view_fuel_del', isLoggedIn, function(req, res){
		FuelDel.
    find().
    sort( '-updated_at' ).
...
app.get(  '/view_site_mat_del', isLoggedIn, function(req, res){
		SiteMDel.
    find().
    sort( '-updated_at' ).
...
app.get(  '/view_eco', isLoggedIn, function(req, res){
		EMCout.
    find().
    sort( '-updated_at' ).
...
app.post( '/index_app', function(req, res, next){
	Todo.
...
app.post( '/index_populate_sites', function(req, res, next){
		var arr = [];
...
app.post( '/sites_id_query', function(req, res, next){
		var limit = 10;
	...
	}).select('site_address site_id ihs_site_id').limit(10).exec(function(err, locations) {
		if (err) {
			return res.status(500).json(err);
		}
	else{
		res.status(200).json(locations);
		//res.status(200).send(JSON.stringify(locations, null, 4));
	}
	});
...
app.post( '/create', isLoggedIn, function(req, res, next){
	...
	var x = convertToLower(req.body.surname);
	var y = convertToLower(req.body.first_name);
	var z = convertToLower(req.body.other_name);
		new Todo({
...
app.post( '/create_site', isLoggedIn, function(req, res, next){
		new Site({
      user_id    : req.cookies.user_id,
      id    : req.body.id,
...
app.get(  '/destroy/:id', isLoggedIn, function(req, res, next){
...
app.get(  '/destroy_site/:id', isLoggedIn, function(req, res, next){
...
app.get(  '/edit_sites/:id', isLoggedIn, function(req, res){
		var x = {};
	var m = req.params.id;
	Site.
    findOne({_id: m}).
...
app.get(  '/view_field_dgs/:id/:id2?', isLoggedIn, function(req, res){
		Todo.
...
app.get(  '/view_field_fuel_del/:id/:id2?', isLoggedIn, function(req, res){
		Todo.
    findOne({makasa_id: req.params.id2}).
    sort( '-updated_at' ).
    exec( function ( err, todos ){
...
app.get(  '/view_field_site_mat_del/:id/:id2?', isLoggedIn, function(req, res){
		Todo.
    findOne({makasa_id: req.params.id2}).
    sort( '-updated_at' ).
    exec( function ( err, todos ){
...
app.get(  '/view_field_eco/:id/:id2?', isLoggedIn, function(req, res){
		Todo.
    findOne({makasa_id: req.params.id2}).
    sort( '-updated_at' ).
    exec( function ( err, todos ){
...
app.post( '/update_site/:id', isLoggedIn, function(req, res, next){
		 Site.findById( req.params.id, function ( err, site ){
